...
export async function sendBookingEmails({ name, company, email, phone, notes, cart, teamEmails }) {
  // In sandbox/testing, send everything to sandbox address to avoid rate limiting
  const recipientEmails = isSandbox() ? ['sandbox@mailtrap.io'] : teamEmails
...
export async function sendEnquiryEmails({ name, company, email, phone, message, teamEmails }) {
  // In sandbox/testing, send everything to sandbox address to avoid rate limiting
  const recipientEmails = isSandbox() ? ['sandbox@mailtrap.io'] : teamEmails
